...
def student_results(student_id):
    if not session.get("logged_in"):
        return redirect(url_for("login"))
    db = get_db()
    results = db.execute("""SELECT results.quiz_id, results.score, quizzes.quiz_date, quizzes.subject FROM results
        JOIN quizzes ON results.quiz_id = quizzes.id WHERE results.student_id = ?""", (student_id,)).fetchall()
